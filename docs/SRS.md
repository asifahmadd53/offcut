...
#### 9.5.3 The diagram is a technical cutting drawing, not a coloured rectangle

The sheet diagram is drawn as an SVG so it reads like a shop drawing:
a panel saw operator expects dimension lines, numbered cuts and
hatching, not flat colour blocks.

- **Sheet outline.** A thick neutral stroke around the whole sheet
  (or, for a leftover, the whole physical parent sheet --- see
  Leftover detail below).
...
leftover itself highlighted as a solid-bordered focus block. On
Cutting plan and Job detail, the diagram and its side panel (legend,
"Sheet used", the leftovers list) sit side by side from small-tablet
width up; on a phone the side panel drops below the diagram instead,
so neither ever crowds or overlaps the other.
